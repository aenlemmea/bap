...
public class UserController {
  ...
  public AuthorizeResponseDTO login(@Valid @RequestBody LoginRequestDTO requestDTO, HttpServletResponse response) {
    //return authorizeService.login(requestDTO);
    AuthorizeServiceResultDTO authorizeServiceResultDTO =
            authorizeService.login(requestDTO);

    // NOTE: Change in deployment
    ResponseCookie cookie = ResponseCookie.from(JWT_COOKIE,
            authorizeServiceResultDTO.token())
            .httpOnly(true)
            .secure(false) // https
            .path("/")
